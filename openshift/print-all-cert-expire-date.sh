...
##
## - This scrpit is designed to run with NOPASS sudo permission user as it reads files under /etc/origin directory
...
CERT_FILES=$(sudo find /etc/origin/{master,node} -type f \( -name '*.crt' -o -name '*pem' \))
for f in $CERT_FILES; do
  echo "- $f"
  sudo cat $f | show_cert
done
...
for f in $KUBECONFIG_FILES; do
  echo "- $f"
  sudo awk '/cert/ {print $2}' $f | base64 -d | show_cert
done
